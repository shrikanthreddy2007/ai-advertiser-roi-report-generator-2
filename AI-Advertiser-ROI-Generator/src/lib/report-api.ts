...
export const generateReportFn = createServerFn()
  .validator((data: any) => data)
  .handler(async ({ data }) => {
    // Dynamically import @google/generative-ai to be safe for bundle-splitting
...
`;
    ...
    try {
      ...
      // Compute dynamic local metrics
      const impressionsNum = Number(data.impressions) || 0;
      const clicksNum = Number(data.clicks) || 0;
      const conversionsNum = Number(data.conversions) || Math.round(clicksNum * 0.12);
      const budgetNum = Number(data.budget) || 1; // avoid divide by zero
      
      const ctr = impressionsNum > 0 ? ((clicksNum / impressionsNum) * 100).toFixed(2) : "0.00";
      
      // Simple ROI calculation: estimate value of conversions vs budget
      const estimatedValue = conversionsNum * 150;
      const calculatedRoiVal = (estimatedValue / budgetNum).toFixed(1);
      ...
      } else if (data.platform === "Search (SEM)") {
        aiRecommendations = [
      ...
      } else if (data.platform === "Connected TV (CTV)") {
        aiRecommendations = [
